...
new_vars = """    @State private var displayDoc: PDFDocument
    ...
    @State private var isShipped: Bool = false
    @AppStorage("panel_showMaterial") private var showMaterial = true"""
if old_vars in text: text = text.replace(old_vars, new_vars)
...
if old_zstack in text: text = text.replace(old_zstack, new_zstack)
...
if old_bottom_bar in text: text = text.replace(old_bottom_bar, new_bottom_bar)
...
if old_task in text: text = text.replace(old_task, "")
...
if old_funcs_block in text: text = text.replace(old_funcs_block, "")

# 7. Delete ALL of API/Overrides and View Mode Components up to PDFKit wrapper
text = re.sub(r'// MARK: - API / Overrides.*?// MARK: - PDFKit Wrapper\n\n', '// MARK: - PDFKit Wrapper\n\n', text, flags=re.DOTALL)

# 8. Clean up PDFKitView to not have autoCrop/overrides.
pdfkit_pattern = r'private struct PdfKitView: UIViewRepresentable \{.*?class Coordinator.*?\}\n\}'

clean_pdf_kit = """private struct PdfKitView: UIViewRepresentable {
...
text = re.sub(pdfkit_pattern, clean_pdf_kit, text, flags=re.DOTALL)
